...
def build_contract(
	...
	for image, role in zip(images, roles):
		...
		assets.append(ImageAsset(
			id=uuid4(), original_filename=image.filename,
			storage_key=image.file_path.replace("\\", "/"), content_type="image/tiff",
			format=AssetFormat.GEOTIFF if suffix == ".tif" else AssetFormat.TIFF,
			role=role, sensor=SensorType.UNKNOWN,
		))
	request = AnalysisRequest(
		question=question, asset_ids=[asset.id for asset in assets],
...
def create_contract_analysis(
	analysis: Analysis, images: list[Image], question: str, requested_capability: str,
	db: Session, storage_root: str | Path,
) -> Analysis:
	...
	analysis.request_data = {"request": request.model_dump(mode="json"), "assets": [asset.model_dump(mode="json") for asset in assets]}
	analysis.plan_data = plan.model_dump(mode="json")
	analysis.question = question
	db.add_all(Execution(analysis_id=analysis.id, step=step.id, specialist=step.specialist.value, status="pending") for step in plan.steps)
	db.flush()
	return analysis
...
def execute_analysis(analysis: Analysis, db: Session, storage_root: str | Path) -> Analysis:
	...
	for specialist_result in result.specialist_results:
		db.add(Result(analysis_id=analysis.id, result_type=specialist_result.specialist.value, data=specialist_result.model_dump(mode="json")))
	steps = {execution.step: execution for execution in db.scalars(select(Execution).where(Execution.analysis_id == analysis.id)).all()}
