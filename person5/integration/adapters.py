...
def _failed(invocation: SpecialistInvocation, message: str, code: str) -> SpecialistResult:
    return SpecialistResult(
        ...
        provenance={"provider": "person5.integration", "failure": code},
        error_code=code,
    )
...
class PreprocessingAdapter:
    ...
    def run(self, invocation: SpecialistInvocation) -> SpecialistResult:
        try:
            paths = {
                asset.role.value: _path(self.provider.storage_root, asset.storage_key)
...
class VisionAdapter:
    ...
    def run(self, invocation: SpecialistInvocation) -> SpecialistResult:
        ...
        task = (
            ...
            if invocation.plan.task.value == "caption"
            else VisionTask(invocation.plan.task.value)
        )
        request = VisionRequest(
            image_reference=_path(self.storage_root, asset.storage_key),
            task=task,
            prompt=invocation.request.question,
            analysis_id=invocation.request.id,
            step_id=invocation.step.id,
        )
        ...
        if adapted is None:
            return _failed(
                invocation,
                "Vision provider did not return an analysis ID.",
                "vision_contract_error",
            )
        return adapted.model_copy(
            update={
                "confidence": result.confidence,
                "confidence_method": "provider_reported" if result.confidence is not None else None,
            }
        )
...
class ChangeAdapter:
    ...
    def run(self, invocation: SpecialistInvocation) -> SpecialistResult:
        try:
            before_path = _path(self.storage_root, _role_key(invocation, "before"))
            after_path = _path(self.storage_root, _role_key(invocation, "after"))
            
            result = self.provider.detect(before_path, after_path)
            mask_filename = f"{invocation.request.id}_change_mask.png"
            mask_path = self.storage_root / mask_filename
            
            import rasterio
            import numpy as np
            
            # Save the boolean mask as a grayscale PNG
            with rasterio.open(
                mask_path, "w", driver="PNG",
                height=result.change_mask.shape[0], width=result.change_mask.shape[1],
                count=1, dtype=rasterio.uint8
            ) as dst:
                dst.write((result.change_mask.astype(np.uint8) * 255)[None, ...])
                
            specialist_result = result.to_specialist_result(invocation.request.id, invocation.step.id)
            answer_dict = json.loads(specialist_result.answer)
            answer_dict["mask_key"] = mask_filename
            answer_dict["bounds"] = result.before.bounds
            specialist_result.answer = json.dumps(answer_dict, sort_keys=True)
...
class FusionAdapter:
    def run(self, invocation: SpecialistInvocation) -> SpecialistResult:
        optical = next(
            (
                result
                for result in invocation.previous_results
                if result.specialist is Specialist.OPTICAL_SAR
            ),
            None,
        )
        if optical is None:
            return SpecialistResult(
                ...
                limitations=["Optical/SAR output was unavailable."],
                provenance={"provider": "person5.fusion_adapter"},
        ...
        return SpecialistResult(
            ...
            status=SpecialistStatus.COMPLETED,
            answer=json.dumps(
                {"optical_sar_step": optical.step_id, "fused": False}, sort_keys=True
            ),
            limitations=[
                "No additional learned fusion interpretation is configured; "
                "the specialist output is retained verbatim."
            ],
            provenance={
                "provider": "person5.fusion_adapter",
                "source": optical.provenance.get("provider", "person4"),
            },
        )
