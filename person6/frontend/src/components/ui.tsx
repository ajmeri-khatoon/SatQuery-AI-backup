...
export function Panel({ children, title, eyebrow, action, className = "", style }: { children: ReactNode; title?: string; eyebrow?: string; action?: ReactNode; className?: string; style?: React.CSSProperties }) {
  return <section className={`panel ${className}`} style={style}><div className="panel-heading">{title && <div><p className="eyebrow">{eyebrow}</p><h2>{title}</h2></div>}{action}</div>{children}</section>;
}
